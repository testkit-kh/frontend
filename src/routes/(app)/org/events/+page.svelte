<script lang="ts">
	...
	const editable = $derived(
		current != null && current.status !== 'completed' && current.status !== 'cancelled'
	);
	...
	async function create() {
		...
		try {
			...
			error = err instanceof ApiError ? err.message : 'Не удалось создать мероприятие';
		} finally {
			busy = false;
		}
	}
	...
	async function cancel() {
		if (!current || busy || !editable) return;
		if (!confirm('Отменить мероприятие? Записавшиеся увидят статус «Отменено».')) return;
		busy = true;
		error = null;
		try {
			const updated = await events.cancel(current.id);
			items = items.map((e) => (e.id === updated.id ? updated : e));
		} catch (err) {
			error = err instanceof ApiError ? err.message : 'Не удалось отменить';
		} finally {
			busy = false;
		}
	}
...
<div class="flex h-full flex-col sm:flex-row">
	<aside class="flex w-full shrink-0 flex-col border-slate-200 bg-white sm:w-80 sm:border-r">
		<header class="flex items-center justify-between gap-2 border-b border-slate-200 p-4">
			<div>
			...
			<button
				...
				class="flex min-h-9 items-center gap-1 rounded-full bg-slate-900 px-3 text-xs font-medium text-white hover:bg-slate-700"
			>
		...
		{:else if items.length === 0}
			<p class="p-4 text-sm text-slate-500">
				Пока пусто. Создайте выезд кнопкой выше — или подтвердите точку в очереди, тогда мероприятие
				появится само.
			</p>
		{:else}
	...
	<div class="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6">
		...
		{#if creating}
			<div class="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4">
				<div class="flex items-center justify-between">
					<h3 class="text-sm font-medium text-slate-700">Новое мероприятие</h3>
					<button
						type="button"
				...
				<label class="flex flex-col gap-1 text-xs text-slate-500">
					...
					<input
						bind:value={newTitle}
						placeholder="Уборка берега у кордона"
		...
		{:else if !current}
			<p class="text-sm text-slate-500">Выберите мероприятие слева или создайте новое.</p>
		{:else}
			<div class="flex flex-col gap-6">
					...
					<div class="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4">
						...
						<div class="flex flex-wrap gap-2">
							...
							<button
								...
								disabled={busy}
								onclick={cancel}
								class="rounded-full border border-red-200 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-50 disabled:opacity-60"
							>
				...
				{:else}
					<p class="text-sm text-slate-500">Мероприятие отменено. Редактирование недоступно.</p>
				{/if}
			</div>
		{/if}
	</div>
